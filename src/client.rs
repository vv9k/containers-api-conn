use crate::{
    build_request, get_response_string, stream_json_response, stream_response, Compat, Error,
...
impl<E: From<Error> + From<serde_json::Error>> RequestClient<E> {
        ...
        B: Into<Body>,
    {
        ...
        match response.status() {
            StatusCode::SWITCHING_PROTOCOLS => {
                Ok(hyper::upgrade::on(response).await.map_err(Error::from)?)
            }
            _ => Err(E::from(Error::ConnectionNotUpgraded)),
        }
    }
}
